...
Page.getPage = async (key) => {
    ...
    const rows = await dbConn.query("SELECT pageKey, title, content, dateModified, username,email FROM `page` JOIN user ON user.userId = `page`.lastEditUser WHERE pageKey = ?", [key]);
    dbConn.end();
